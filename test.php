...
<body>
    ...
    <main>
        ...
        <br>
        <input type="hidden" class="courseid" id='courseId' value="<?php echo $courseid ?>" />
        <input type="hidden" class="chapterId" id='chapterId' value="<?php echo $chapter_id ?>" />
        <h4 style="position: fixed;width: 80px;
    background-color: #883a8e;color: white;bottom: -10px;right: 0;font-size: 15px;justify-content: center;display: flex;padding: 10px;font-weight: 600;font-family: 'Open Sans Regular';">
          ...
          <div class="toast" style="opacity: 1;display:none;">
            ...
            <div class="toast-body"> 
                ...
                <ul> 
                    ...
                    <li style="display: flex;justify-content: space-around;margin: 10px;">
                    <button class="show-data">Show reviews</button> 
                     
                    <a href="#" class="try-again"> <button> Try again </button> </a>
                    <a href="#" class="nextt" style="display:none;"> <button >Next Chapter</button> </a>
                    </li> 
                </ul> 
            </div> 
          </div>
          ...
          <div style="background-color: white;" class="reviews">
            <ol class="questions">
                <?php
                    include 'backend_data/init.php';
                    $answers = "";
                    $datamhen = "SELECT * FROM `questions` WHERE question_type LIKE '%".trim($chapter)."%' ORDER BY RAND() LIMIT 20";
                    $result = $conn->query($datamhen);
                    ...
                    function checkimage($image){
                        ...
                        if(in_array($imgExt,$allowedExt)){
                         $data = ' <div class="image-question" style="display: flex;justify-content: center;">
            ...
            <button class="btn btn-primary submit" style="font-size: 14px;font-weight: 600;height: 50px;font-family: 'Open Sans Regular';display: grid;width: 100%;margin-top: 60px;background-color: #883a8e;border: 1px solid;align-items: center;"> Submit</button>
          </div>
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        setTimeout(function(){
            ...
            if (eval(score/20 * 100) >= 60 ){
                var courseId = $('#courseId').val();
                var chapterId = $('#chapterId').val();
                $.ajax({
                    type:'POST',
                    url:'push.php',
                    data:'score='+eval(score/20 * 100)+'&courseid='+courseId+'&chapterid='+chapterId,
